...
import { Role, rolesService } from '@/lib/api-services-admin';
...
const schema = z.object({
  ...
  abbreviation: z.string().trim().min(1, 'Sigla obrigatória').max(20),
  status: z.enum(['A', 'I']),
});
...
export function RoleForm({ item, onSave, onCancel }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (item) {
        await rolesService.update(item.id, data);
        toast.success('Perfil atualizado!');
      } else {
        await rolesService.create(data as Omit<Role, 'id'>);
        toast.success('Perfil criado!');
      }
      queryClient.invalidateQueries({ queryKey: ['admin-roles'] });
      onSave();
    } catch (error: any) {
      applyApiErrors(error, setError, { fallbackMessage: 'Erro ao salvar perfil.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2">
        <Label>Nome</Label>
        <Input {...register('name')} placeholder="Nome do perfil" />
        {errors.name && <p className="text-sm text-destructive">{errors.name.message}</p>}
      </div>

      <div className="space-y-2">
        <Label>Sigla</Label>
        <Input {...register('abbreviation')} placeholder="Ex.: ADM" />
