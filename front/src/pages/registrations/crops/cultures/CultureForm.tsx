...
import { Culture, culturesService } from '@/lib/api-services';
...
export function CultureForm({ item, onSave, onCancel }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (item) {
        await culturesService.update(item.id, data);
        toast.success('Cultura atualizada!');
      } else {
        await culturesService.create(data as Omit<Culture, 'id'>);
