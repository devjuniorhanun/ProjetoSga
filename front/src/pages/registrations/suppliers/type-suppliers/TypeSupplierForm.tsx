...
import { TypeSupplier, typeSuppliersService } from '@/lib/api-services';
...
interface Props {
  item?: TypeSupplier | null;
  onSave: () => void;
  onCancel: () => void;
}

export function TypeSupplierForm({ item, onSave, onCancel }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (item) {
        await typeSuppliersService.update(item.id, data);
        toast.success('Tipo de fornecedor atualizado!');
      } else {
        await typeSuppliersService.create(data as Omit<TypeSupplier, 'id'>);
        toast.success('Tipo de fornecedor criado!');
      }
      queryClient.invalidateQueries({ queryKey: ['type-suppliers'] });
      onSave();
    } catch (error: any) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2">
        <Label>Nome</Label>
        <Input {...register('name')} placeholder="Ex: Transportadora" />
