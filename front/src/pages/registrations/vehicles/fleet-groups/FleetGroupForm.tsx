...
import { FleetGroup, fleetGroupsService } from '@/lib/api-services-vehicles';
...
interface Props { item?: FleetGroup | null; onSave: () => void; onCancel: () => void; }

export function FleetGroupForm({ item, onSave, onCancel }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try {
      if (item) { await fleetGroupsService.update(item.id, data); toast.success('Grupo de frota atualizado!'); }
      else { await fleetGroupsService.create(data as Omit<FleetGroup, 'id'>); toast.success('Grupo de frota criado!'); }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2"><Label>Nome</Label><Input {...register('name')} placeholder="Ex: Caminhões" />{errors.name && <p className="text-sm text-destructive">{errors.name.message}</p>}</div>
