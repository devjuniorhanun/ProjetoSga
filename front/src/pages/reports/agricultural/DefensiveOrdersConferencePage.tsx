...
export default function DefensiveOrdersConferencePage() {
  ...
  return (
    <div className="space-y-6">
      <style>{`@page { size: A4 landscape; margin: 10mm; } @media print { html, body { width: 297mm; min-height: 210mm; } body * { visibility: hidden !important; } .defensive-print, .defensive-print * { visibility: visible !important; } .defensive-print { position: absolute; inset: 0; width: 100%; } .print-hide { display: none !important; } }`}</style>
      ...
      {report && <div className="defensive-print space-y-5">
        ...
        <Card><CardHeader><CardTitle>Ordens do talhão</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>O.S.</TableHead><TableHead>Data</TableHead><TableHead>Cultura</TableHead><TableHead>Área</TableHead><TableHead>Bombas recomendadas</TableHead><TableHead>Bombas utilizadas</TableHead><TableHead>Diferença de bombas</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{report.orders.map((order) => <TableRow key={String(order.id)} className="cursor-pointer print:cursor-default" onClick={() => navigate(`/entries/agricultural/defensives/order/${order.id}`)}><TableCell className="font-medium">{order.os_number}</TableCell><TableCell>{formatDate(order.application_date)}</TableCell><TableCell>{order.culture_name || '-'}</TableCell><TableCell>{number(order.area)}</TableCell><TableCell>{number(order.recommended_pump)}</TableCell><TableCell>{number(order.used_bomb)}</TableCell><TableCell>{order.pump_difference_percentage == null ? '-' : `${number(order.pump_difference_percentage)}%`}</TableCell><TableCell>{statusLabels[order.status] ?? order.status}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>

        {report.orders.map((order) => <Card key={String(order.id)}><CardHeader><CardTitle>O.S. {order.os_number} — {formatDate(order.application_date)}</CardTitle></CardHeader><CardContent className="space-y-3"><div className="grid grid-cols-3 gap-2 text-sm"><span>Safra: {order.crop_name}</span><span>Cultura: {order.culture_name || '-'}</span><span>Talhão: {order.field_name}</span><span>Operação: {order.operation_name}</span><span>Área: {number(order.area)} ha</span><span>Status: {statusLabels[order.status] ?? order.status}</span><span>Bombas recomendadas: {number(order.recommended_pump)}</span><span>Bombas utilizadas: {number(order.used_bomb)}</span><span>Diferença de bombas: {order.pump_difference_percentage == null ? '-' : `${number(order.pump_difference_percentage)}%`}</span><span>Operadores: {order.operators.map((operator) => operator.name).filter(Boolean).join(', ') || '-'}</span><span>Frotas: {order.operators.map((operator) => operator.fleet).filter(Boolean).join(', ') || '-'}</span></div>{order.observation && <p className="text-sm">Observações: {order.observation}</p>}</CardContent></Card>)}
