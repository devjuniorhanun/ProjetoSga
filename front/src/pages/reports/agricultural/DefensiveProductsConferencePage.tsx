...
export default function DefensiveProductsConferencePage() {
  ...
  return <div className="space-y-6">
    ...
    {report && <div className="defensive-products-print space-y-5"><div className="text-center"><h2 className="text-xl font-bold">Conferência de Produtos por Operação</h2><p>{report.crop.name} • {report.operation.name}</p><p>{report.order_count} ordem(ns) • {report.field_count} talhão(ões) • Área considerada: {number(report.considered_area)} ha</p></div><Card><CardHeader><CardTitle>Produtos utilizados</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Produto</TableHead><TableHead>Quantidade recomendada</TableHead><TableHead>Dose recomendada</TableHead><TableHead>Quantidade usada</TableHead><TableHead>Quantidade usada / área</TableHead></TableRow></TableHeader><TableBody>{report.products.map((product) => <TableRow key={String(product.product_id)}><TableCell>{product.product_name}</TableCell><TableCell>{number(product.recommended_quantity)}</TableCell><TableCell>{number(product.recommended_dose)}</TableCell><TableCell>{number(product.used_quantity)}</TableCell><TableCell>{number(product.used_per_area)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card><div className="pt-16 text-center"><div className="mx-auto w-80 border-t border-black pt-2">Assinatura do responsável</div></div><ReportPrintFooter /></div>}
  </div>;
}
