...
const schema = z.object({
  ...
  cost_center_id: z.string().min(1, 'Centro de custo obrigatório'),
...
export function PayAccountForm({ item, onSave, onCancel }: Props) {
  const queryClient = useQueryClient();
  const [loading, setLoading] = useState(false);
  const [valueDisplay, setValueDisplay] = useState(item ? formatCurrencyBRL(item.value) : '');

  const { data: centers = [] } = useQuery({ queryKey: ['cost-centers', 'active-options'], queryFn: () => costCentersService.getAll({ status: 'A' }) });
  ...
  const centerOptions = useMemo(
    () => centers
      .filter((c) => c.status === 'A' || String(c.id) === String(item?.cost_center_id))
      .map((c) => ({ value: String(c.id), label: c.name || String(c.id) })),
    [centers, item?.cost_center_id]
  );
  ...
  const onSubmit = async (data: FormData) => {
    const belongsToProducer =
      administrativeCenterOptions.some((o) => o.value === data.administrative_center_id);
    ...
    try {
      if (item) {
        await payAccountsService.update(item.id, payload);
        toast.success('Conta atualizada!');
      } else {
        await payAccountsService.create(payload as PayAccountPayload);
