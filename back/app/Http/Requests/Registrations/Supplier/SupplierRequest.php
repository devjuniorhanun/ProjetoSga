...
namespace App\Http\Requests\Registrations\Supplier;
...
// Executa uma operação utilizando uma dependência ou propriedade da classe.
                'corporate_reason')->ignore($this->route('supplier')?->id ?? $this->route('supplier'))
// Define o item atual da coleção ou configuração.
            ],
            // Valida o campo conforme as regras de negócio.
// Define este campo ou configuração na estrutura atual.
...
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
                Rule::unique('suppliers',
// Executa uma operação utilizando uma dependência ou propriedade da classe.
                'fantasy_name')->ignore($this->route('supplier')?->id ?? $this->route('supplier'))
// Define o item atual da coleção ou configuração.
            ],
            // Valida o campo conforme as regras de negócio.
// Restringe o campo a um conjunto fechado de valores permitidos.
            'type' => ['sometimes', Rule::in(['F','J'])],
            // Valida o campo conforme as regras de negócio.
// Define este campo ou configuração na estrutura atual.
            'cpf_cnpj' => [
// Exige que o campo seja informado.
...
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
                Rule::unique('suppliers',
// Executa uma operação utilizando uma dependência ou propriedade da classe.
                'cpf_cnpj')->ignore($this->route('supplier')?->id ?? $this->route('supplier'))
// Define o item atual da coleção ou configuração.
            ],
            // Valida o campo conforme as regras de negócio.
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'rg_ie' => ['required', 'string', 'max:255', Rule::unique('suppliers','rg_ie')->ignore($this->route('supplier')?->id ?? $this->route('supplier'))],
            // Valida o campo conforme as regras de negócio.
// Restringe o campo a um conjunto fechado de valores permitidos.
