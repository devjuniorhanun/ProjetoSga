...
class EntryInvoiceController extends Controller
{
    ...
    private function rules(?int $ignoreId = null): array
    {
        return [
            ...
            'access_key'=>['nullable','digits:44',Rule::unique('entry_invoices','access_key')->ignore($ignoreId)],'document_model'=>'nullable|string|max:10','invoice_number'=>'required|string|max:100','series'=>'nullable|string|max:20','issue_date'=>'required|date','entry_date'=>'required|date','operation_nature'=>'nullable|string|max:255',
            ...
            'items.*.allocations'=>'required_unless:entry_type,SEED|array|min:1','items.*.allocations.*.stock_location_id'=>'required|integer|exists:stock_locations,id','items.*.allocations.*.plot_field_id'=>'nullable|integer|exists:plot_fields,id','items.*.allocations.*.fuel_station_id'=>'required_if:entry_type,FUEL,LUBRICANT|nullable|integer|exists:fuel_stations,id','items.*.allocations.*.batch'=>'required_if:entry_type,DEFENSIVE|nullable|string|max:100','items.*.allocations.*.manufacturing_date'=>'required_if:entry_type,DEFENSIVE|nullable|date','items.*.allocations.*.expiration_date'=>'required_if:entry_type,DEFENSIVE|nullable|date|after_or_equal:items.*.allocations.*.manufacturing_date','items.*.allocations.*.quantity'=>'required|numeric|gt:0',
            'items.*.seed_lots'=>'required_if:entry_type,SEED|array|min:1','items.*.seed_lots.*.culture_id'=>'required|integer|exists:cultures,id','items.*.seed_lots.*.variety_culture_id'=>'required|integer|exists:variety_cultures,id','items.*.seed_lots.*.stock_location_id'=>'required|integer|exists:stock_locations,id','items.*.seed_lots.*.lot_number'=>'required|string|max:100','items.*.seed_lots.*.sieve'=>'required|string|max:50','items.*.seed_lots.*.quantity'=>'required|numeric|gt:0','items.*.seed_lots.*.unit'=>'required|string|max:20','items.*.seed_lots.*.manufacturing_date'=>'nullable|date','items.*.seed_lots.*.expiration_date'=>'nullable|date|after_or_equal:items.*.seed_lots.*.manufacturing_date',
